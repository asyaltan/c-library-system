...
void dosyayaKaydet() {
    ...
    if (fKullanici != NULL) {
        ...
        while (gk != NULL) {
            fprintf(fKullanici, "%s,%s,%d\n", gk->kullaniciAdi, gk->sifre, gk->rol);
            gk = gk->sonraki;
        }
        fclose(fKullanici);
    }
...
void dosyadanYukle() {
    ...
    if (fKullanici != NULL) {
        ...
        while (fscanf(fKullanici, " %[^,],%[^,],%d", kadi, sifre, &rol) == 3) {
    ...
    if (fOdunc != NULL) {
        ...
        while (fscanf(fOdunc, " %[^,],%[^,],%ld,%d,%d", oIsbn, oKadi, &oTarih, &oSure, &oIade) == 5) {
            Odunc *yeni = (Odunc*)malloc(sizeof(Odunc));
...
int main(){
    ...
    aktifRol = girisYap();
    if(aktifRol == 0){
        printf("Hatali giris! Lutfen programi yeniden baslatin.\n");
    }
    else{
        ...
        while (secim != 0) {
            ...
            if (secim == 1) {
                ...
                dosyayaKaydet();
                printf("Tum veriler kaydedildi. Sistemden cikiliyor. Iyi gunler!\n");
            }
            else {
